...
class SB:
    def __init__(self) -> None:
        ...
        # 启动时的用户 cwd，供 auto-workspace 使用。早期捕获，之后即使
        # stdio/日志被重定向也不受影响。GA_LAUNCH_CWD 让 ga.cmd 等包装脚本
        # 在切目录前预存原始 cwd；直接 python 启动时退回 os.getcwd()。
        self._launch_cwd: str = os.environ.get('GA_LAUNCH_CWD') or os.getcwd()
    ...
    def _auto_ws_reject_reason(self, p: str) -> str:
        """自动 workspace 的白名单式拒绝检查。返回非空 = 拒绝原因；空串 = 通过。
        比 validate_path 更严：额外拦 GA 目录内 / 家/桌面/下载/文档 / 盘根 / 只读。"""
        import tempfile as _tf
        if not p or not os.path.isdir(p):
            return '无有效 cwd'
        norm = os.path.normcase(os.path.abspath(p))
        ga_root = os.path.normcase(workspace_cmd._REPO_ROOT)
        if norm == ga_root or norm.startswith(ga_root + os.sep):
            return 'cwd 位于 GA 目录内'
        home = os.path.normcase(os.path.expanduser('~'))
        too_broad = {home,
                     os.path.normcase(os.path.join(home, 'Desktop')),
                     os.path.normcase(os.path.join(home, 'Downloads')),
                     os.path.normcase(os.path.join(home, 'Documents')),
                     os.path.normcase(os.path.join(home, '桌面'))}
        if norm in too_broad:
            return '目录过于宽泛（家/桌面/下载/文档）'
        if len(norm.replace('\\', '/').strip('/').split('/')) <= 1:
            return '目录过于宽泛（盘根）'
        try:
            with _tf.TemporaryFile(dir=p): pass
        except OSError as e:
            return f'目录不可写: {e}'
        ok, msg = workspace_cmd.validate_path(p)
        return '' if ok else msg

    def _try_auto_workspace(self) -> None:
        """启动时按用户 cwd 自动绑定 workspace。严格白名单，任何一步失败都放弃
        并留一行提示（不影响 TUI 启动）。GA_NO_AUTO_WS=1 关闭本行为。"""
        if os.environ.get('GA_NO_AUTO_WS'):
            return
        if self._ws_name:                        # 已被续接恢复或其他路径绑过
            return
        cwd = self._launch_cwd
        reason = self._auto_ws_reject_reason(cwd)
        if reason:
            self.commit([_DIM + f'ℹ 未自动绑定 workspace：{reason}' + _RST])
            return
        r = workspace_cmd.prepare(cwd)
        if not r.get('ok'):
            self.commit([_DIM + f'ℹ 自动 workspace 失败：{r.get("error")}' + _RST])
            return
        if r.get('warning'):
            self.commit([_DIM + f'ℹ 自动 workspace 警告：{r["warning"]}（放弃自动绑定）' + _RST])
            return
        self._bind_workspace(r)
        disp = os.path.basename(cwd.rstrip('/\\')) or cwd
        self.commit([_DIM + f'✅ 自动进入 workspace「{disp}」' + _RST])
